...
const BLOCK_LABELS = {
  ...
  pricing: "Preise", gallery: "Galerie", contact: "Kontaktformular", footer: "Footer"
};
...
function EKYBuilder() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('eky_builder_v4');
      if (saved) {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      ...
      try {
        localStorage.setItem('eky_builder_v4', JSON.stringify({ version: 4, template, blocks, business, colors }));
  ...
  const saveProject = async () => {
    if (window.electronAPI) {
      const r = await window.electronAPI.saveProject({ version: 4, template, blocks, business, colors });
  ...
  const exportWebsiteFolder = async () => {
    const files = generateSiteFiles(blocks, business, colors);
  ...
  useEffect(() => {
    window.ekyApp = {
      exportHtml, exportWebsiteFolder, saveProject, openProject,
          ...
          {leftTab === 'seo' && (
            <>
              ...
              {[
                ...
                { k: 'conversionGoal', l: 'Conversion-Ziel' },
                { k: 'calendly', l: '📅 Calendly-/Buchungslink' }
              ].map(f => (
                <div key={f.k} style={{ marginBottom: 10 }}>
...
function generateHtml(blocks, b, colors) {
  const title = b.seoTitle || `${b.name} — ${b.tagline}`;
  const desc = b.seoDescription || `${b.name} in ${b.city}. ${b.tagline}. ${(b.areaServed || []).slice(0, 3).join(', ')}. Tel: ${b.phone}`;
  const slugDomain = (b.name || 'website').toLowerCase().replace(/[^a-z0-9äöüß]+/gi, '-').replace(/^-|-$/g, '');
  const url = b.website || `https://${slugDomain}.de`;
...
${JSON.stringify(schema, null, 2)}
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .hero-img{aspect-ratio:4/5;background:linear-gradient(135deg,var(--a),var(--p));border-radius:10px;box-shadow:0 24px 80px rgba(0,0,0,.14);}
    .trust{background:var(--p);color:#fff;padding:14px 48px}.trust-inner{max-width:1200px;margin:0 auto;display:flex;justify-content:center;flex-wrap:wrap}.trust-item{padding:6px 18px;border-right:1px solid rgba(255,255,255,.18);font-size:13px;font-weight:700;display:flex;gap:8px}.trust-item:last-child{border-right:0}.usps{padding:80px 48px;background:#fff}.usps-w{max-width:1120px;margin:0 auto}.usp-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:18px}.usp-card{display:flex;gap:16px;background:var(--panel);border:1px solid var(--b);border-left:4px solid var(--a);border-radius:12px;padding:22px}.usp-no{width:36px;height:36px;border-radius:8px;background:var(--a);color:var(--p);display:flex;align-items:center;justify-content:center;font-weight:900;flex:0 0 auto}.usp-card h3{font-size:17px;margin-bottom:7px}.usp-card p{font-size:14px;color:var(--m);line-height:1.65}.process{padding:80px 48px;background:var(--p);color:#fff}.process-w{max-width:1120px;margin:0 auto}.process h2{color:#fff;text-align:center}.process-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:18px}.step{text-align:center;padding:22px 14px}.step-no{width:52px;height:52px;border-radius:50%;background:var(--a);color:var(--p);display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-weight:900}.step h3{font-size:16px;margin-bottom:8px}.step p{font-size:13px;line-height:1.65;opacity:.78}.contact-sec{padding:80px 48px;background:#fff}.contact-w{max-width:980px;margin:0 auto;display:grid;grid-template-columns:.9fr 1.1fr;gap:38px}.contact-card{background:var(--panel);border:1px solid var(--b);border-radius:14px;padding:26px}.contact-card label{display:block;font-size:12px;font-weight:700;margin:12px 0 6px}.contact-card input,.contact-card textarea{width:100%;border:1px solid var(--b);border-radius:8px;padding:12px 14px;font:inherit;background:#fff}.contact-card textarea{min-height:120px;resize:vertical}.contact-submit{border:0;background:var(--p);color:#fff;border-radius:8px;padding:13px 18px;font-weight:800;margin-top:14px;width:100%}.contact-facts{font-size:14px;color:var(--m);line-height:1.8;margin-top:16px;}
    ...
    @media(max-width:900px){
      ...
      .usp-grid,.process-grid,.contact-w{grid-template-columns:1fr 1fr;}
    ...
    @media(max-width:480px){
      ...
      h2{font-size:22px;}
      .hero,.features,.cta-sec,.faq-sec,.split-sec,.testi,.pricing-sec,.usps,.process,.contact-sec{padding:36px 14px;}
      .hero-badge{font-size:10px;padding:4px 10px;}
      .hero-btns{flex-direction:column;}
      .btn-primary,.btn-secondary{width:100%;text-align:center;}
      .cta-btns{flex-direction:column;}
      .cta-btn,.cta-tel{width:100%;text-align:center;}
      .feat-grid,.usp-grid,.process-grid,.contact-w{grid-template-columns:1fr;}
      .trust{padding:10px 14px}.trust-inner{justify-content:flex-start}.trust-item{border-right:0;padding:4px 8px;font-size:11px}.btn-p,.btn-s{width:100%;text-align:center;}
...
function generateSiteFiles(blocks, b, colors) {
  ...
  const readme = `EKY Builder v4 Export\n=====================\n\nDateien:\n- index.html: Hauptseite\n- impressum.html: rechtliche Vorlage, vor Livegang prüfen\n- datenschutz.html: rechtliche Vorlage, vor Livegang prüfen\n- sitemap.xml / robots.txt: SEO-Basis\n\nWichtig: Mailto-Kontaktformulare sind nur eine Übergangslösung. Für echte Kunden-Websites bitte Backend/Formspree/CRM anschließen.\n`;
  return { 'index.html': generateHtml(blocks, b, colors), 'impressum.html': generateLegalPage('impressum', b, colors), 'datenschutz.html': generateLegalPage('datenschutz', b, colors), 'sitemap.xml': sitemap, 'robots.txt': robots, 'README.txt': readme };
}
...
function renderBlockHtml(block, b, colors) {
  ...
  if (block.type === 'trust') return `    <section class="trust" aria-label="Vertrauen"><div class="trust-inner">
${((block.data?.Elemente) || []).map(el => `      <div class="trust-item"><span>${esc(el.Icon || '✓')}</span><span>${esc(el.Text || '')}</span></div>`).join('\n')}
    </div></section>`;

  if (block.type === 'usps') return `    <section class="usps" id="vorteile"><div class="usps-w"><h2>${esc(block.data?.Titel || 'Warum Kunden uns wählen.')}</h2><div class="usp-grid">
${((block.data?.Elemente) || []).map((el, i) => `      <article class="usp-card"><div class="usp-no">${i + 1}</div><div><h3>${esc(el.Titel || '')}</h3><p>${esc(el.Text || '')}</p></div></article>`).join('\n')}
    </div></div></section>`;

  if (block.type === 'process') return `    <section class="process" id="ablauf"><div class="process-w"><h2>${esc(block.data?.Titel || 'So läuft die Zusammenarbeit ab.')}</h2><div class="process-grid">
${((block.data?.Elemente) || []).map(el => `      <article class="step"><div class="step-no">${esc(el.Nummer || '')}</div><h3>${esc(el.Titel || '')}</h3><p>${esc(el.Text || '')}</p></article>`).join('\n')}
    </div></div></section>`;
...
${Array.from({ length: d.images || 6 }).map((_, i) => `        <div class="gal-item" role="img" aria-label="Referenzbild ${i + 1}"></div>`).join('\n')}
  ...
  if (block.type === 'contact') return `    <section class="contact-sec" id="anfrage"><div class="contact-w"><div><p class="eyebrow">Kontakt</p><h2>${esc(d.title || block.data?.Titel || 'Projekt anfragen')}</h2><p class="hero-sub">${esc(d.sub || block.data?.Untertitel || 'Beschreiben Sie kurz Ihr Anliegen. Wir melden uns schnellstmöglich zurück.')}</p><div class="contact-facts"><strong>${esc(b.name)}</strong><br>${esc(b.street)} · ${esc(b.postalCode)} ${esc(b.city)}<br><a href="tel:${(b.phone || '').replace(/\s/g, '')}">${esc(b.phone)}</a><br><a href="mailto:${esc(b.email)}">${esc(b.email)}</a></div></div><form class="contact-card" action="mailto:${esc(b.email)}" method="post" enctype="text/plain">
${((d.fields || block.data?.Felder) || []).map((f, i) => `      <label for="field-${i}">${esc(f.label || f.Label || 'Feld')}${(f.required || f.Pflicht) === 'ja' ? ' *' : ''}</label>${(f.type || f.Typ) === 'textarea' ? `<textarea id="field-${i}" name="${esc(f.label || f.Label || 'Nachricht')}" ${(f.required || f.Pflicht) === 'ja' ? 'required' : ''}></textarea>` : `<input id="field-${i}" name="${esc(f.label || f.Label || 'Feld')}" type="text" ${(f.required || f.Pflicht) === 'ja' ? 'required' : ''}>`}`).join('\n')}
      <label style="display:flex;gap:8px;align-items:flex-start;font-size:12px;font-weight:500;color:var(--m);line-height:1.5"><input type="checkbox" required style="width:auto;margin-top:3px"> Ich stimme der Verarbeitung meiner Angaben zur Kontaktaufnahme zu.</label><button class="contact-submit" type="submit">${esc(d.button || block.data?.Button || 'Anfrage senden')}</button></form></div></section>`;
