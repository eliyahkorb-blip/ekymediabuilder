            ...
            {blocks.map((block, i) => (
              <React.Fragment key={block.id}>
                <div draggable
                  ...
                  <BlockRender block={block} business={business} colors={colors} />
                </div>
...
// BLOCK RENDERER
// ============================================================
function BlockRender({ block, business, colors }) {
  ...
  if (block.type === 'nav') return (
    <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 48px', borderBottom: `1px solid ${colors.primary}22` }}>
      <div style={{ fontFamily: f.serif, fontSize: 22, fontStyle: 'italic' }}>{business.name}</div>
      <div style={{ display: 'flex', gap: 24, fontSize: 13, color: colors.text + '99' }}>
        <span>Leistungen</span><span>Über uns</span><span>Kontakt</span>
      </div>
      <div style={{ background: colors.primary, color: '#fff', padding: '8px 16px', borderRadius: 6, fontSize: 12, fontWeight: 600 }}>
  ...
  if (block.type === 'hero') return (
    <section style={{ padding: '72px 48px 60px', display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 48, alignItems: 'center' }}>
      <div>
        <div style={{ fontFamily: f.mono, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: colors.accent, marginBottom: 16 }}>{d.eyebrow}</div>
        <h1 style={{ fontFamily: f.serif, fontSize: 52, lineHeight: 1.04, letterSpacing: '-0.02em', marginBottom: 20, fontWeight: 400 }} dangerouslySetInnerHTML={{ __html: d.headline }} />
        <p style={{ fontSize: 15, lineHeight: 1.7, color: colors.text + '88', marginBottom: 28, maxWidth: 440 }}>{d.sub}</p>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ background: colors.primary, color: '#fff', padding: '11px 22px', borderRadius: 7, fontSize: 13, fontWeight: 600 }}>{d.cta1} →</div>
          <div style={{ padding: '11px 22px', border: `1px solid ${colors.primary}44`, borderRadius: 7, fontSize: 13, fontWeight: 600 }}>{d.cta2}</div>
        </div>
      </div>
      <div style={{ aspectRatio: '4/5', background: `linear-gradient(135deg, ${colors.accent}44, ${colors.primary})`, borderRadius: 10 }} />
    </section>
  );

  if (block.type === 'features') return (
    <section style={{ padding: '56px 48px', background: colors.primary + '08', borderTop: `1px solid ${colors.primary}18` }}>
      <h2 style={{ fontFamily: f.serif, fontSize: 30, marginBottom: 28, fontWeight: 400 }}>{d.title}</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {(d.items || []).map((it, i) => (
          <div key={i} style={{ background: '#fff', padding: 22, borderRadius: 8, border: `1px solid ${colors.primary}18` }}>
            <div style={{ width: 30, height: 30, background: colors.accent, borderRadius: 6, marginBottom: 12 }} />
            <h3 style={{ fontSize: 15, marginBottom: 6, fontWeight: 600 }}>{it.title}</h3>
            <p style={{ fontSize: 12, color: colors.text + '77', lineHeight: 1.6 }}>{it.text}</p>
          </div>
        ))}
      </div>
    </section>
  ...
  if (block.type === 'split') return (
    <section style={{ padding: '72px 48px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
      <div style={{ aspectRatio: '4/3', background: `linear-gradient(135deg, ${colors.primary}, ${colors.accent})`, borderRadius: 10 }} />
      <div>
        <div style={{ fontFamily: f.mono, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: colors.accent, marginBottom: 12 }}>{d.eyebrow}</div>
        <h2 style={{ fontFamily: f.serif, fontSize: 34, lineHeight: 1.1, marginBottom: 16, fontWeight: 400 }}>{d.headline}</h2>
        <p style={{ fontSize: 15, lineHeight: 1.7, color: colors.text + '88' }}>{d.text}</p>
      </div>
    </section>
  );
  ...
  if (block.type === 'trust') return (
    <section style={{ background: p, padding: isMobile ? '12px 16px' : '14px 40px' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: isMobile ? 6 : 0, justifyContent: 'center', maxWidth: 960, margin: '0 auto' }}>
        {(d.Elemente || []).map((el, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: isMobile ? '3px 10px' : '0 18px', borderRight: !isMobile && i < (d.Elemente.length - 1) ? '1px solid rgba(255,255,255,.2)' : 'none', fontSize: isMobile ? 11 : 12, color: '#fff', fontWeight: 600, whiteSpace: 'nowrap' }}>
  ...
  if (block.type === 'usps') return (
    <section style={{ padding: isMobile ? '36px 16px' : '64px 40px', background: '#fff' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <h2 style={{ fontFamily: serif, fontSize: isMobile ? 24 : 34, fontWeight: 400, marginBottom: isMobile ? 22 : 32, textAlign: 'center' }}>{d.Titel}</h2>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? '1fr 1fr' : 'repeat(2,1fr)', gap: isMobile ? 12 : 20 }}>
          {(d.Elemente || []).map((el, i) => (
  ...
  if (block.type === 'process') return (
    <section style={{ padding: isMobile ? '36px 16px' : '64px 40px', background: p, color: '#fff' }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        <h2 style={{ fontFamily: serif, fontSize: isMobile ? 24 : 34, fontWeight: 400, marginBottom: isMobile ? 24 : 36, textAlign: 'center', color: '#fff' }}>{d.Titel}</h2>
  ...
  if (block.type === 'cta') return (
    <section style={{ padding: '72px 48px', background: colors.primary, color: '#fff', textAlign: 'center' }}>
      <h2 style={{ fontFamily: f.serif, fontSize: 38, marginBottom: 12, fontWeight: 400 }}>{d.headline}</h2>
      <p style={{ fontSize: 15, opacity: 0.8, marginBottom: 28 }}>{d.sub}</p>
      <div style={{ display: 'inline-block', background: colors.accent, color: colors.primary, padding: '13px 28px', borderRadius: 7, fontSize: 14, fontWeight: 700 }}>
        {business.calendly ? `${d.cta} →` : `${d.cta} →`}
      </div>
    </section>
  );

  if (block.type === 'faq') return (
    <section style={{ padding: '72px 48px', background: colors.primary + '06', borderTop: `1px solid ${colors.primary}18` }}>
      <h2 style={{ fontFamily: f.serif, fontSize: 30, marginBottom: 28, fontWeight: 400 }}>{d.title}</h2>
      <div style={{ maxWidth: 700 }}>
        {(d.items || []).map((it, i) => (
          <div key={i} style={{ borderBottom: `1px solid ${colors.primary}20`, padding: '18px 0' }}>
            <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>{it.q}</h3>
            <p style={{ fontSize: 13, color: colors.text + '88', lineHeight: 1.6 }}>{it.a}</p>
          </div>
        ))}
      </div>
    </section>
  ...
  if (block.type === 'testimonial') return (
    <section style={{ padding: '72px 48px', textAlign: 'center' }}>
      <div style={{ fontSize: 48, color: colors.accent, marginBottom: 12, fontFamily: f.serif }}>"</div>
      <p style={{ fontFamily: f.serif, fontSize: 26, fontStyle: 'italic', fontWeight: 400, lineHeight: 1.4, marginBottom: 20, maxWidth: 680, marginLeft: 'auto', marginRight: 'auto' }}>{d.quote}</p>
      <div style={{ fontSize: 13, color: colors.text + '77', fontWeight: 600 }}>— {d.author}</div>
    </section>
  );

  if (block.type === 'pricing') return (
    <section style={{ padding: '72px 48px', background: colors.primary + '08' }}>
      <h2 style={{ fontFamily: f.serif, fontSize: 30, marginBottom: 28, fontWeight: 400 }}>{d.title}</h2>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${(d.plans || []).length},1fr)`, gap: 16 }}>
        {(d.plans || []).map((p, i) => (
          <div key={i} style={{ background: '#fff', padding: 28, borderRadius: 8, border: i === 1 ? `2px solid ${colors.accent}` : `1px solid ${colors.primary}18` }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: colors.accent, marginBottom: 6 }}>{p.name}</div>
            <div style={{ fontFamily: f.serif, fontSize: 30, fontWeight: 400 }}>{p.price}</div>
          </div>
        ))}
      </div>
    </section>
  ...
  if (block.type === 'gallery') return (
    <section style={{ padding: '56px 48px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {Array.from({ length: d.images || 6 }).map((_, i) => (
          <div key={i} style={{ aspectRatio: 1, background: `linear-gradient(${i * 45}deg, ${colors.accent}66, ${colors.primary})`, borderRadius: 8 }} />
        ))}
      </div>
    </section>
  );
  ...
  if (block.type === 'footer') return (
    <footer style={{ padding: '48px 48px 28px', background: '#0a0a0a', color: '#fff' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 28, marginBottom: 28 }}>
        <div>
          <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 20, fontStyle: 'italic', marginBottom: 10 }}>{business.name}</div>
          <p style={{ fontSize: 12, color: '#888', lineHeight: 1.6 }}>{business.tagline}</p>
          <p style={{ fontSize: 11, color: '#555', marginTop: 8 }}>Erstellt von EKY Media · ekymedia.de</p>
        </div>
        <div>
          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#666', marginBottom: 8 }}>Kontakt</div>
          <div style={{ fontSize: 12, lineHeight: 1.8 }}>{business.phone}<br />{business.email}</div>
        </div>
        <div>
          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#666', marginBottom: 8 }}>Adresse</div>
          <div style={{ fontSize: 12, lineHeight: 1.8 }}>{business.street}<br />{business.postalCode} {business.city}</div>
        </div>
        <div>
          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#666', marginBottom: 8 }}>Rechtliches</div>
          <div style={{ fontSize: 12, lineHeight: 1.8 }}>Impressum<br />Datenschutz<br />AGB</div>
        </div>
      </div>
      <div style={{ fontSize: 11, color: '#555', borderTop: '1px solid #222', paddingTop: 14 }}>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    @media(max-width:900px){
      .hero,.split-sec{grid-template-columns:1fr;padding:48px 24px;gap:28px;}
      h1{font-size:42px;} h2{font-size:28px;}
      .features,.cta-sec,.faq-sec,.pricing-sec{padding:52px 24px;}
      .feat-grid,.pricing-grid{grid-template-columns:1fr;}
      .gal-grid{grid-template-columns:repeat(2,1fr);}
      .footer-grid{grid-template-columns:1fr 1fr;}
      nav.site-nav{padding:16px 24px;flex-wrap:wrap;}
