...
function EKYBuilder() {
  ...
  const loadProjectPayload = (data) => {
    const { template: t, blocks: b, business: biz, colors: c, media: m } = data || {};
    setTemplate(t || 'elektriker');
    setBlocks(b || cloneDeep(TEMPLATES.elektriker.blocks));
    setBusiness(biz || cloneDeep(TEMPLATES.elektriker.business));
    setColors(c || cloneDeep(TEMPLATES.elektriker.colors));
    setMedia(normalizeMedia(m));
    setSelectedId(null);
  };

  const saveProject = async () => {
    const payload = { version: 7.1, template, blocks, business, colors, media: normalizeMedia(media), exportedAt: new Date().toISOString() };
    const fileName = `${slugify(business.name || 'eky-projekt') || 'eky-projekt'}.eky.json`;
    if (window.electronAPI) {
      const r = await window.electronAPI.saveProject(payload);
      if (r?.success) { setLastSaved(Date.now()); setSaveStatus('saved'); showToast('✓ Projekt gespeichert'); }
    } else {
      browserDownload(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' }), fileName);
      setLastSaved(Date.now()); setSaveStatus('saved'); showToast('✓ Projektdatei heruntergeladen');
    }
  };
  ...
  const openProject = async () => {
    if (window.electronAPI) {
      ...
      if (r?.success && r.data) {
        ...
        loadProjectPayload(r.data);
        showToast('✓ Projekt geöffnet');
      }
    } else {
      const input = document.createElement('input');
      input.type = 'file';
      input.accept = '.json,.eky.json,application/json';
      input.onchange = async () => {
        ...
        try {
          ...
          console.error(err);
          showToast('Projektdatei konnte nicht gelesen werden');
        }
      };
      input.click();
    }
  ...
  const exportWebsiteFolder = async () => {
    ...
    const folderName = (business.name || 'website').toLowerCase().replace(/[^a-z0-9äöüß]+/gi, '-').replace(/^-|-$/g, '') || 'website';
    if (window.electronAPI?.saveSiteFolder) {
      ...
      try {
        ...
        Object.entries(files).forEach(([name, value]) => {
          if (value && typeof value === 'object' && value.encoding === 'base64') zip.file(name, value.content, { base64: true });
          else zip.file(name, String(value ?? ''));
        });
        const blob = await zip.generateAsync({ type: 'blob' });
        browserDownload(blob, `${folderName}-website-export.zip`);
        showToast('✓ Website-ZIP heruntergeladen');
      } catch (err) {
        console.error(err);
        showToast('ZIP-Export fehlgeschlagen');
      }
    }
  };
